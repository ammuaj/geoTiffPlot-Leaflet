...
L.LeafletGeotiff = L.ImageOverlay.extend({
    ...
    setColorAlpha: function (alphav) {
        this.options.alpha = alphav;
        this._reset();
    },
    ...
    _drawImage: function () {
        ...
        if (self.raster.hasOwnProperty('data')) {
            ...
            ctx.clearRect(0, 0, plotCanvas.width, plotCanvas.height);
                         ...
                         if (this.options.colorScaling==true) {
                        ...
                        if (this.options.circle==true) {
                            ...
                            for (var y=0;y<self.raster.height;y=y+stride) {
                                for (var x=0;x<self.raster.width;x=x+stride) {
                                    ...
                                    if (self.raster.data[rasterIndex]>=self.options.displayMin && self.raster.data[rasterIndex]<=self.options.displayMax) { //Ignore missing values
                                        ...
                                        //L.circle([currentLat,currentLng],200)

                         ctx.beginPath();
                         ctx.arc(xProjected, yProjected, this.options.radius, 0, Math.PI * 2);
                         ctx.fill();
                         ctx.closePath();
                                    }
                                }
